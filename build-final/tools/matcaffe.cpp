...
static mxArray* mydo_forward(const mxArray* const bottom) {
  ...
  int id = find(blob_names.begin(), blob_names.end(), "fc7")-blob_names.begin();
  //int id=16;
  
  
  
  mxArray* mx_out2 = mxCreateCellMatrix(1, 1);
  
  int x = blobs.size();
  
  mxArray* mx_blob2 = mxCreateNumericMatrix(blobs[id]->count(),
        1, mxSINGLE_CLASS, mxREAL);
  
  float* mydata_ptr = reinterpret_cast<float*>(mxGetPr(mx_blob2));
  //memcpy(mydata_ptr, blobs[id]->cpu_data(),
//   /       sizeof(float) * blobs[id]->count());
  ...
  switch (Caffe::mode()) {
    case Caffe::CPU:
      ...
      break;
    case Caffe::GPU:
      cudaMemcpy(mydata_ptr, blobs[id]->gpu_data(),
          sizeof(float) * blobs[id]->count(), cudaMemcpyDeviceToHost);
      break;
    default:
      LOG(FATAL) << "Unknown Caffe mode.";
    }  // switch (Caffe::mode
  
  mxSetCell(mx_out2, 1, mx_blob2);
  return mx_blob2;
}
...
static void init(MEX_ARGS) {
  ...
  init_key = caffe_rng_rand();  // NOLINT(caffe/random_fn)
  //init_key = rand();  
  if (nlhs == 1) {
    plhs[0] = mxCreateDoubleScalar(init_key);
  }
...
static void myforward(MEX_ARGS) {
  if (nrhs != 1) {
    LOG(ERROR) << "Only given " << nrhs << " arguments";
    mexErrMsgTxt("Wrong number of arguments");
  }

  plhs[0] = mydo_forward(prhs[0]);
}
